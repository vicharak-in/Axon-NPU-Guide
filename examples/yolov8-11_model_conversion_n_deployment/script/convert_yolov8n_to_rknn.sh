...
# ============================================================================
# YOLOv8 to RKNN Automated Conversion Pipeline
# ============================================================================
# This script automates the entire process of converting YOLOv8 PyTorch models
# to RKNN format.
# ============================================================================
...
OPSET_VERSION=19
EXPORT_IMGSZ=640  # Export image size

# RKNN conversion settings
GRAPH_SIZES="320,320 480,480 640,640"  # Space-separated H,W pairs
PLATFORM="rk3588"  # rk3588, rk3576, etc.
ENABLE_QUANTIZATION=false
DATASET_FILE=""  # Path to dataset.txt for quantization

# Dynamic model settings
DYNAMIC_BATCH=false  # Set to true if you want dynamic batch dimension
...
Automate YOLOv8 to RKNN conversion pipeline.

Options:
    -m, --model NAME            Model name (default: yolov8n)
    -s, --sizes "H,W H,W..."   Graph sizes (default: "320,320 480,480 640,640")
    ...
    -b, --dynamic-batch         Enable dynamic batch dimension
    --opset VERSION            ONNX opset version (default: 19)
    --imgsz SIZE               Export image size (default: 640)
    --url URL                   Download model from URL
    -h, --help                  Show this help message

Examples:
    # Basic conversion
    $0 -m yolov8n -s "640,640"
    
    # Multiple sizes with quantization
    $0 -m yolov8s -s "320,320 640,640 1280,736" -q -d dataset.txt
    
    # Different platform
    $0 -m yolov8n -s "640,640" -p rk3576
...
REQUIRED_SCRIPTS=("utils/removenodes.py" "utils/static2dynamic.py" "utils/onnx2rknn.py")
...
if [ "$RKNN_INSTALLED" = "yes" ]; then
    ...
    # The toolkit requires onnx.mapping which was removed in newer versions
...
print_header "Step 4: Export PyTorch to ONNX"
...
# ============================================================================
# Step 2: Remove DFL Nodes
# ============================================================================

print_header "Step 5: Remove DFL Nodes"

source venv-onnx/bin/activate

MODEL_MODIFIED="modified_${MODEL_NAME}.onnx"
...
print_info "Removing DFL nodes..."
$PYTHON_CMD utils/removenodes.py "$MODEL_ONNX" "$MODEL_MODIFIED"
...
# ============================================================================
# Step 3: Convert to Dynamic ONNX
# ============================================================================

print_header "Step 6: Convert to Dynamic ONNX"

source venv-onnx/bin/activate

MODEL_DYNAMIC="modified_${MODEL_NAME}_dynamic.onnx"

if [ -f "$MODEL_DYNAMIC" ]; then
    print_warning "Dynamic model already exists: $MODEL_DYNAMIC"
    rm "$MODEL_DYNAMIC"
fi

print_info "Converting to dynamic model..."
if [ "$DYNAMIC_BATCH" = true ]; then
    $PYTHON_CMD utils/static2dynamic.py --model "$MODEL_MODIFIED" --hw --output "$MODEL_DYNAMIC"
else
    $PYTHON_CMD utils/static2dynamic.py --model "$MODEL_MODIFIED" --hw --no-batch --output "$MODEL_DYNAMIC"
fi
print_success "Dynamic conversion complete: $MODEL_DYNAMIC"

deactivate
echo ""

# ============================================================================
# Step 4: Convert to RKNN
# ============================================================================

print_header "Step 7: Convert to RKNN"

source venv-rknn/bin/activate

print_info "Converting to RKNN format..."
print_info "  Platform: $PLATFORM"
print_info "  Graph sizes: $GRAPH_SIZES"
print_info "  Quantization: $ENABLE_QUANTIZATION"

# Build command
RKNN_CMD="$PYTHON_CMD utils/onnx2rknn.py $MODEL_DYNAMIC --platform $PLATFORM --graphsz $GRAPH_SIZES"
...
RKNN_FILE=$(ls -t modified_${MODEL_NAME}_dynamic*.rknn 2>/dev/null | head -1)
...
echo "  1. ONNX (static):      $MODEL_ONNX"
echo "  2. ONNX (modified):    $MODEL_MODIFIED"
echo "  3. ONNX (dynamic):     $MODEL_DYNAMIC"
echo "  4. RKNN (final):       $RKNN_FILE"
echo ""
echo "Next steps:"
