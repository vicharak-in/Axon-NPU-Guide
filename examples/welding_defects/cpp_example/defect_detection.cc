...
cv::Mat DefectDetection::preprocess(const cv::Mat& image) {
    cv::Mat img;
...
void DefectDetection::inference(cv::Mat& frame, cv::Mat& image) {
    int ret;
...
void DefectDetection::postprocess(cv::Mat& frame, rknn_output* outputs, int iter) {
...
void DefectDetection::inferenceWorker(int workerId) {
    int ret;
    int model_len = 0;
    char* model;
    rknn_context ctxI;
...
void DefectDetection::postprocessWorker(int workerId) {
...
void DefectDetection::run(const std::string& streamPath) {
    ...
    for(int i=0; i<numInferenceWorkers; ++i) {
        inferenceThreads.emplace_back(&DefectDetection::inferenceWorker, this, i);        
    }
    ...
    for(int i=0; i<numPostprocessWorkers; ++i) {
        postprocessThreads.emplace_back(&DefectDetection::postprocessWorker, this, i);
    }
    std::thread streamThread(&DefectDetection::streamer, this);
